...
3. COMPLETE link verification — confirm ALL wiki links in the note
   resolve to existing files. A single dangling wiki link FAILS.
   Markdown path links used in Source attribution are excluded from dangling checks.
...
- Ignore markdown path links (for example `Source: [name](ops/queue/archive/.../file.md)`) in this check
