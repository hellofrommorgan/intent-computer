...
| Pipeline queue | Automated processing state — what the SYSTEM needs to process | Pipeline skills | Automatic: /reduce, /process, /reflect |
...
| Pipeline has pending tasks | "Pipeline has {N} pending tasks. Run /process to process them." |
...
| Read by | /next (priority #1) | /process (phase routing) |
