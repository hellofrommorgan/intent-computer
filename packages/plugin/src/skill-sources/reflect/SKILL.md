...
External loop mode for /process:
...
- What do navigation breadcrumbs in `ops/observations/navigation/` reveal?
...
### Phase 6: Write Navigation Breadcrumbs to Ops

Process breadcrumbs must not be written into {vocabulary.topic_map} files.

When you discover a useful navigation path or dead end:
1. Write a breadcrumb note to `ops/observations/navigation/[map-slug].md`
2. Keep the map file synthesis-only (`Core Ideas`, `Tensions`, `Gaps/Open Questions`)
3. Use deterministic filename `[map-slug].md` (kebab-case map title)

Breadcrumb format:
```markdown
# Navigation Breadcrumbs: [Map Title]

- YYYY-MM-DD: [path explored] -> [insight or dead end]
```
...
- Breadcrumb written: ops/observations/navigation/[map-slug].md
...
- Navigation breadcrumbs are written to ops, not maps
...
- Write navigation breadcrumbs to `ops/observations/navigation/[map-slug].md`
...
When invoked with `--handoff`, output this structured format at the END of the session. This enables external loops (/process) to parse results and update the task queue.
...
When running in handoff mode via /process, the prompt includes the task file path. After completing the workflow, update the `## {vocabulary.reflect}` section of that task file with:
...
When running interactively (NOT via /process), YOU must advance the phase in the queue. /process handles this automatically, but interactive sessions do not.
