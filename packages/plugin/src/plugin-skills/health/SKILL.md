...
Markdown path links (for source attribution) are not part of this check.

**How to check:**

```bash
# Extract all wiki links from all markdown files
# Exclude fenced code blocks and markdown path links
...
| Condition | Level |
|-----------|-------|
| Any dangling wiki link (target does not exist) | FAIL |
...
[6] Processing Throughput ........ WARN
    ...
    Recommendation: run /reduce on oldest inbox items or /process for end-to-end processing
```
...
4. Verify map hygiene: no `Agent Notes` section and no process-log/session-log paragraphs embedded in the map
...
| Map contains `Agent Notes` section or process-log/session-log patterns | WARN (move logs to `ops/observations/navigation/`) |
...
| Inbox items | >= 3 items | Consider /reduce or /process |
...
| Processing throughput | /reduce or /process | Process inbox items to improve ratio |
